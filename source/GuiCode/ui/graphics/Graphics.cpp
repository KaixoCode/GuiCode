...
namespace Graphics
{
    ...
    Vec3<float> m_HSVMults{ 0, 0, 0 };
    void OffsetHsv(const Vec3<float>& offsets) 
    {
        m_HSVMults.h = offsets.h;
        m_HSVMults.s = offsets.s;
        m_HSVMults.v = offsets.v;
    }
    ...
    void RunCommands(::CommandCollection& d)
    {
        ...
        for (int i = 0; i < d.Get().size(); i++)
        {
            ...
            if (!m_IgnoreDraws)
            {
                ...
                case Fill: 
                {
                    auto hsv = rgb2hsv(a->fill / 255.0);
                    hsv.h = std::fmod(hsv.h + m_HSVMults.h, 360);
                    hsv.s = constrain(hsv.s + m_HSVMults.s, 0, 1);
                    hsv.v = constrain(hsv.v + m_HSVMults.v, 0, 1);
